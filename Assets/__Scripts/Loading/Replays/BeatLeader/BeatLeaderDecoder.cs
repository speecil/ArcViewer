...
public class AsyncReplayDecoder
{
    ...
    private async Task<Dictionary<string, byte[]>> DecodeCustomData(Stream stream)
    {
        ...
        int count = await DecodeInt(stream);
        for(int i = 0; i < count; i++)
        {
            string key = await DecodeString(stream);
            byte[] value = await DecodeByteArray(stream);
            result[key] = value;
        }
        return result;
    }
...
public static class ReplayDecoder
{
    ...
    private static List<Frame> DecodeFrames(byte[] buffer, ref int pointer)
    {
        ...
        for(int i = 0; i < length; i++)
        {
            Frame frame = DecodeFrame(buffer, ref pointer);
            if(frame.time != 0 && (result.Count == 0 || frame.time != result[result.Count - 1].time))
            {
                result.Add(frame);
            }
    ...
    private static Dictionary<string, byte[]> DecodeCustomData(byte[] buffer, ref int pointer)
    {
        ...
        for(int i = 0; i < count; i++)
        {
            string key = DecodeString(buffer, ref pointer);
            byte[] value = DecodeByteArray(buffer, ref pointer);
            result[key] = value;
        }
        return result;
    }
